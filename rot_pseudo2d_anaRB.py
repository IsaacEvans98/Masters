...
problem.add_equation("  T_ref*( Pr*dt(s) - dy(dy(s)) - dz(sz) ) + theta*(m+1)*sz \
                        ...
                        + 2*Y*( dy(v)*dy(v) + uz*uz + vz*dy(w) - (1/3)*(dy(v)+wz)*(dy(v)+wz) + (1/2)*(dy(u)*dy(u) + uz*uz + vz*vz + dy(w)*dy(w)) )")

# Flux equations for use in analysis outputs
problem.add_equation("  dz(L_buoy) = -s*rho_ref*w")
problem.add_equation("  dz(L_diss) = -2*rho_ref*( dy(v)*dy(v) + uz*uz + vz*dy(w) - (1/3)*(dy(v)+wz)*(dy(v)+wz) + (1/2)*(dy(u)*dy(u) + uz*uz + vz*vz + dy(w)*dy(w)) )")
...
analysis.add_task("integ(s,'y')/Ly", layout='g', name='<s>_x')
...
analysis.add_task("integ( (-1)*rho_ref*( u*uz + v*( vz+dy(w) ) + (2/3)*w*( 2*wz - dy(w) ) ), 'y')*((Pr*Pr*theta)/Ra)/Ly",layout='g',name='L_visc')
...
analysis.add_task(" integ( integ( 2*rho_ref*( dy(v)*dy(v) + uz*uz + vz*dy(w) - (1/3)*(dy(v)+wz)*(dy(v)+wz) + (1/2)*(dy(u)*dy(u) + uz*uz + vz*vz + dy(w)*dy(w)) ) \
...
# analysis.add_task(" T*rho_ref*u*cos_lat ", layout='g', name="z_cor")
# analysis.add_task(" T*rho_ref*w*cos_lat ", layout='g', name="x_cor")
