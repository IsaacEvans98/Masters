...
problem.add_equation("  rho_ref*( dt(u) - (4/3)*dx(dx(u)) - dz(uz) - (1/3)*dx(wz) + (Ta)**(0.5)*(w*cos(phi) - v*sin(phi))  )  + dx(p) \
                        - dz_rho_ref*( uz + dx(w) ) \
                        = rho_ref*(-u*dx(u) - w*uz )   ")

#y-component of the momentum equation
problem.add_equation(" rho_ref*(dt(v) - dx(dx(v)) - dz(vz)  + (Ta)**(0.5)*sin(phi)*u  ) \
                       - dz_rho_ref*(vz) \
                       = rho_ref*( -u*dx(v) - w*vz )   ")


# z-component of the momentum equation
problem.add_equation("  rho_ref*T_ref*( dt(w) - X*s - (4/3)*dz(wz) - dx(dx(w)) - (1/3)*dx(uz) + (Ta)**(0.5)*u*cos(phi) ) + T_ref*dz(p) + theta*m*p \
                        + (2/3)*theta*m*rho_ref*( dx(u) - 2*wz ) \
